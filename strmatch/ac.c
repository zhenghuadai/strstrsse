...
int strmat_ac_match(char  **pattern_ary, int num_patterns, char *text,
                    int stats)
{
...
/*    newmatch = alloc_match();
    if (newmatch == NULL) {
      ...
      ac_free(acstruct);
      printf("Memory Error:  Ran out of memory.\n");
      return 0;
    }
...
//  print_matches(text, NULL, 0, matchlist, matchcount);
  ...
  if (stats) {
    ...
    printf("      Sum of Pattern Sizes:       %d\n", total_length);
    printf("      Number of Created Edges:    %d\n",
            acstruct->prep_new_edges);
    printf("      Number of Traversed Edges:  %d\n",
            acstruct->prep_old_edges);
    printf("      Failure Link Comparisons:   %d\n",
            acstruct->prep_fail_compares);

    printf("\n   Searching:\n");
    printf("      Text Length:                %d\n", N);
    printf("      Number of Compares:         %d\n", acstruct->num_compares);
    printf("      Avg. Compares per Position: %.2f\n",
            (float) acstruct->num_compares / (float) N);
    printf("\n");
    printf("      Tree Edges Traversed:       %d\n",
            acstruct->edges_traversed);
    printf("      Fail Links Traversed:       %d\n", acstruct->num_failures);
    printf("      Output Link Traversed:      %d\n",
