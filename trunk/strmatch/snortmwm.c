               ...
               void * data )
{
    ...
    //  ConvCaseToUpperEx( S, S, n ); /* Copy and Convert to Upper Case */
//ConvCaseToUpper(T,strlen(T));
      if( ps->msMethod == MTH_BM )
      {
         /* Boyer-Moore  */
...
printf("ooooooooooooooooooooooooooooooooooooooo:");
      ...
      {
         /* Wu-Manber */
printf("ldkjfsllllllllllllllllllllllllllllllllllllllll");
         return ps->search( ps, T, n, T, match, data );
      }
}
...
** Test driver 
*/
void snortwm(char *Tt,char *Patts[],int s)
{
...
*/
   ...
   {
	   npats++;
	   patArray[npats]=Patts[i];
	   len=strlen(Patts[i]);
	    mwmAddPatternEx( ps, (unsigned char*)Patts[i], len, nocase, 0, 0, (void*)npats /* ID */, 3000 /* IID -internal id*/ );
